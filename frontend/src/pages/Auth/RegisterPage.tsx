...
export const RegisterPage = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: ({ user, tokens }) => {
      setAuth(user, tokens)
      toast.success(t('register.welcome_toast', { name: user.first_name }))
      const next = searchParams.get('next')
      const target = next && next.startsWith('/') && !next.startsWith('//') ? next : '/app'
      navigate(target)
    },
    onError: (e: any) => {
      const msg = e?.response?.data?.message || t('register.error_generic')
      toast.error(msg)
    },
  })
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden px-4 py-8">
      ...
      <div className="absolute -top-40 -right-40 h-[400px] w-[400px] rounded-full bg-violet/8 blur-[80px] pointer-events-none" />
      <div className="absolute -bottom-40 -left-40 h-[400px] w-[400px] rounded-full bg-accent/8 blur-[80px] pointer-events-none" />
      <div className="absolute top-1/4 left-1/3 h-48 w-48 rounded-full bg-primary/6 blur-[60px] pointer-events-none" />
